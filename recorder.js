/*!
 * djApp Recorder v1.0 — WAV PCM16 stereo
...
(function () {
  ...
  if (!OrigCtx) {
    console.warn('[djApp Recorder] AudioContext non disponibile');
    return;
  }
  ...
  class PatchedCtx extends OrigCtx {
    constructor() {
      ...
      if (!state.ctx) {
        state.ctx = this;
        setupTap(this).catch(function (e) { console.error('[djApp Recorder] setup:', e); });
      }
    }
  }
  ...
  async function setupTap(ctx) {
    // Tap gain nodo in parallelo
    var tap = ctx.createGain();
    tap.gain.value = 1.0;
    tap.connect(ctx.destination);   // connessione diretta PRIMA del patch di connect
    state.tap = tap;

    // Ridirigi ogni connect(ctx.destination) verso tap, eccetto il tap stesso
    ...
    tap.connect(worklet);  // worklet in parallelo (non va a destination)
    ...
    console.log('[djApp Recorder] Tap armato, sampleRate=' + ctx.sampleRate);
    renderUI();
  }
  ...
  function encodeWAV(buffers, sampleRate) {
    ...
    dv.setUint32(28, sampleRate * 4, true);    // byte rate = sr * ch * bytes
  ...
  function renderUI() {
    ...
    style.textContent = [
      '#djapp-rec-ui{position:fixed;bottom:16px;right:16px;z-index:2147483647;',
      ...
      'font:500 12px/1 -apple-system,system-ui,sans-serif;box-shadow:0 6px 18px rgba(0,0,0,.4)}',
      '#djapp-rec-ui button{border:0;border-radius:6px;padding:6px 10px;cursor:pointer;',
      'font:600 12px/1 inherit;background:#e8ff47;color:#0d0d0d}',
      '#djapp-rec-ui button.on{background:#ff3344;color:#fff}',
      '#djapp-rec-dot{width:8px;height:8px;border-radius:50%;background:#666}',
      '#djapp-rec-ui.on #djapp-rec-dot{background:#ff3344;animation:djapp-blink 1s infinite}',
      '@keyframes djapp-blink{50%{opacity:.3}}',
      '#djapp-rec-time{font-variant-numeric:tabular-nums;min-width:44px}'
    ].join('');
  ...
  // Mostra UI anche prima del primo AudioContext (pulsante darà alert finché non pronto)
